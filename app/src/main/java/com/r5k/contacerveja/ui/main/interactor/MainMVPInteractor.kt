...
//        billsRepository.loadBiils()
    ...
    override fun createBillAndDefaultDrinks(bill: Bill): Single<DefaulDrinksForBill> {
        return billsRepository.insertBiil(bill).flatMap { t ->  createDefaultsDrinks(t)}
    }

    fun createDefaultsDrinks(billId : Long): Single<DefaulDrinksForBill> {
        ...
        return Single.zip(drinksRepoHelper.insertDrink(drink1),
                        ...
                        Function3<Boolean,Boolean,Boolean,DefaulDrinksForBill>
            {t1, t2, t3 -> DefaulDrinksForBill(t1,t2,t3) })
